...
gcn_dim = int(sys.argv[2])
dropout = float(sys.argv[3])
rec_dropout = float(sys.argv[4])
learning_rate = float(sys.argv[5])
model_location = str(sys.argv[6])
result_location = str(sys.argv[7])
...
print(data.keys())
with codecs.open('paragraph-id-mapping.json', 'r') as f:
...
data_list = {}
key_list =  ['X', 'Y', 'DepEdges', 'SrlEdges']
...
# Adding placeholder
input_x_paragraph = tf.placeholder(tf.int32, shape=[batch_size, paragraph_size], name='input_data_paragragh')
input_x_sentence = tf.placeholder(tf.int32, shape=[batch_size, num_of_sentence, sentence_size], name='input_data_sentence')
input_y_location = tf.placeholder(tf.int32, shape=[batch_size, num_of_sentence, 3], name='input_previous_anno')
...
x=data_list['train'][15][1][0]['is_location_known']
x
...
def get_placeholder_labels(Y):
    ...
    y_location = np.zeros((len(Y), num_of_sentence, 3), np.int3)
    for i, y in enumerate(Y):
        for j, y_sentence in enumerate(y):
            ...
            y_location[i][j][0] = y_known[i][j].index(1)
            y_location[i][j][1] = y_start[i][j]
            y_location[i][j][2] = y_end[i][j]
    return y_known, y_start, y_end, y_location
...
#     print (x_entity)
    ...
    if wLabels:
        y_known, y_start, y_end, y_location = get_placeholder_labels(Y)
        feed_dict[input_y_known] = y_known
        feed_dict[input_y_start] = y_start
        feed_dict[input_y_end] = y_end
        feed_dict[input_y_location] = y_location
...
def createEvalFile(category, start, end, mask, paragraph_id, entity, dtype):
    with codecs.open(result_location+'propara-results-'+dtype+'1.txt', 'w') as f:
...
def fit(sess):
    ...
    best_val_loss = 99
    save_path = model_location+"best_validation.ckpt"
    count = 0
...
index = 9
tf.tile(tf.expand_dims(input_y_location[:,index], 1), [1,174,1])
...
with tf.variable_scope('Embeddings') as scope:
    ...
    embeddings = tf.get_variable('embedding', initializer=embed_init, trainable=True, regularizer=regularizer)
...
for index in range(num_of_sentence):
    ...
    if index==0:
        ...
        previous_location_existence = tf.tile(tf.expand_dims(input_y_location[:,index], 1), [1,174,1])
    entity_sentence_gcn = tf.reduce_mean(entity_sentence_gcn, axis = 2, keepdims = True)
    #print(entity_sentence_gcn)
    #print(previous_location_existence)
    current_entity_sentence_gcn = tf.concat([tf.cast(previous_location_existence, tf.float32) , entity_sentence_gcn], 2)
    #print(current_entity_sentence_gcn)
    current_entity_sentence_gcn_vector = tf.reduce_sum(current_entity_sentence_gcn, 1)
    #print(current_entity_sentence_gcn_vector)
    ...
    final_mask = tf.to_float(tf.logical_and(tf.equal(category_gold, 2), input_x_mask_sentence[:,index]))
...
with tf.Session() as sess:
    saver = tf.train.Saver()
    saver.restore(sess, model_location+"best_validation.ckpt")
