...
def createEvalFile(category, start, end, mask, paragraph_id, entity, dtype):
    with codecs.open('data/propara-results-3GCNs'+dtype+'.txt', 'w') as f:
...
def fit(sess):
    ...
    best_val_loss = 99
    save_path = "models/best_validation.ckpt"
    count = 0
...
with tf.variable_scope('Embeddings') as scope:
    ...
    embedding_entity = tf.squeeze(tf.reduce_mean(tf.nn.embedding_lookup(embeddings, input_x_entity), 1))
...
for index in range(num_of_sentence):
    ...
    with tf.variable_scope('attention-entity-sentence', reuse = tf.AUTO_REUSE) as scope:
        ...
        bilstm_entity_sentence = tf.reduce_mean(bilstm_entity_sentence, 1)
    ...
    with tf.variable_scope('attention-entity-sentence-gcn', reuse = tf.AUTO_REUSE) as scope:
        w_e_s_g = tf.get_variable('w_e_s_g', [entity_sentence_dim, gcn_dim], initializer=tf.contrib.layers.xavier_initializer(), regularizer=regularizer)
        atten_temp_gcn = tf.tensordot(entity_sentence, w_e_s_g, axes=[[1],[0]])
        attention_coeff_gcn = tf.nn.softmax(tf.squeeze(tf.matmul(tf.expand_dims(atten_temp_gcn, 1), tf.transpose(gcn_out2, perm=[0,2,1]))))
        entity_sentence_gcn = tf.multiply(tf.tile(tf.expand_dims(attention_coeff_gcn, 2), [1,1,gcn_dim]),gcn_out2)
    ...
    current_entity_sentence_gcn_vector = tf.reduce_mean(current_entity_sentence_gcn, 1)
...
optimizer = tf.train.AdamOptimizer(3e-4).minimize(total_loss)
...
with tf.Session() as sess:
    saver = tf.train.Saver()
    saver.restore(sess, "models/best_validation.ckpt")
