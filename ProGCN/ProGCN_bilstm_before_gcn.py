...
with open("out1.txt" , "r") as f:
    for line in f:
        with open("para.json", "w") as f1:
            f1.write(line)
        f1.close()
        with open("para.json", "r") as f2:
            x = json.load(f2)
        f2.close()
        l=x['sentences']
...
def createEvalFile(category, start, end, mask, paragraph_id, entity, dtype):
    with codecs.open('data/propara-results_dep-'+dtype+'.txt', 'w') as f:
...
with tf.variable_scope('Bi-LSTM-paragraph') as scope:
    ...
    val, state = tf.nn.bidirectional_dynamic_rnn(fw_cell, bk_cell, embedding_paragraph, sequence_length=input_x_len_paragraph, dtype=tf.float32)
...
gcn_output2 = GCNLayer(gcn_in = bilstm_out_paragraph, in_dim = bilstm_paragraph_dim, gcn_dim = gcn_dim,
