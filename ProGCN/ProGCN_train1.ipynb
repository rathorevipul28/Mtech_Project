...
with open("out1.txt" , "r") as f:
    for line in f:
        with open("para1.json", "w") as f1:
            f1.write(line)
        f1.close()
        with open("para1.json", "r") as f2:
            x = json.load(f2)
        f2.close()
        l=x['sentences']
...
def createEvalFile(category, start, end, mask, paragraph_id, entity, dtype):
    with codecs.open('data/propara-results_srl_a-'+dtype+'.txt', 'w') as f:
...
def fit(sess):
    ...
    best_val_loss = 99
    save_path = "models/best_validation1.ckpt"
    count = 0
...
gcn_output2 = GCNLayer(gcn_in = embedding_paragraph, in_dim = embedding_dim, gcn_dim = gcn_dim,
                      ...
                      regularizer = regularizer,
                      num_layers = gcn_layers,wGate = True, name = "GCN1")
...
with tf.Session() as sess:
    saver = tf.train.Saver()
    saver.restore(sess, "models/best_validation1.ckpt")
