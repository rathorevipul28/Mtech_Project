...
def error_analysis_create_location(labels, predictions):
    tp = fp = tn = fn = 0.0
    len_pred = 0
    len_label = 0
    count = 0
    ...
    for pid,lstTurkerLabels in labels.items():
        for turkerLabels in lstTurkerLabels:
            for x in [x for x in turkerLabels if x.event_type == 'create' and x.to_location != 'unk']:
                pred_loc = predictions[pid][x.participant][x.sid].to_location
                # if pred_loc != 'null' and pred_loc != 'unk':
                len_pred += len(pred_loc.split(" "))
                len_label += len(x.to_location.split(" "))
                count+=1
            #     print (x.participant, pid, pred_loc, "***", x.to_location, "***", location_match(pred_loc, x.to_location))
            #     if int(pred_loc != 'null' and pred_loc != 'unk' and not location_match(pred_loc, x.to_location)):
            #     	print ("false positive!")
            # print ("-------------------------------")
                # tp += int(pred_loc != 'null' and pred_loc != 'unk' and location_match(pred_loc, x.to_location))
                # fp += int(pred_loc != 'null' and pred_loc != 'unk' and not location_match(pred_loc, x.to_location))
                # fn += int(pred_loc == 'null' or pred_loc == 'unk
    # print (len_pred/count, len_label/count)


def error_analysis_destroy_step(labels, predictions):
    tp = fp = tn = fn = 0.0
    # find all created participants and their destroy step
    for pid,lstTurkerLabels in labels.items():
        for turkerLabels in lstTurkerLabels:
            for x in [x for x in turkerLabels if x.event_type == 'destroy' and x.from_location != 'unk']:
                pred_loc = predictions[pid][x.participant][x.sid].from_location
                # if pred_loc != 'null' and pred_loc != 'unk':
                	# print (x.participant, pid, pred_loc, "***", x.from_location, "***", location_match(pred_loc, x.from_location))
                # if int(pred_loc != 'null' and pred_loc != 'unk' and not location_match(pred_loc, x.from_location)):
                	# print ("false positive!")
            # print ("-------------------------------")


def error_analysis_destroy_location(labels, predictions):
    tp = fp = tn = fn = 0.0
    len_pred = 0
    len_label = 0
    count = 0
    ...
    for pid,lstTurkerLabels in labels.items():
        for turkerLabels in lstTurkerLabels:
            for x in [x for x in turkerLabels if x.event_type == 'destroy' and x.from_location != 'unk']:
                ...
                # if pred_loc != 'null' and pred_loc != 'unk':
                len_pred += len(pred_loc.split(" "))
                len_label += len(x.from_location.split(" "))
                count+=1
                # print (x.participant, pid, pred_loc, "***", x.from_location, "***", location_match(pred_loc, x.from_location))
                # if int(pred_loc != 'null' and pred_loc != 'unk' and not location_match(pred_loc, x.from_location)):
                # 	print ("false positive!")
...
def error_analysis_move_location1(labels, predictions):
    tp = fp = tn = fn = 0.0
    len_pred = 0
    len_label = 0
    count = 0
...
# Q10 Participant X moves at step Y, and its location after step Y is known. What is its location after step Y?
def error_analysis_move_location2(labels, predictions):
    tp = fp = tn = fn = 0.0
    len_pred = 0
    len_label = 0
    count = 0
